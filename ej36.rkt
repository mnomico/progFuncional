...
#|
    Hay que hacer una función que verifique que por cada minuto que pase si el tanque se llenó o se vació
|#
...
(define (verificarEstado capacidadTanque diferenciaLitros litrosActuales minutosTranscurridos)
    (define litrosSiguientes (+ litrosActuales diferenciaLitros))   ; Defino los litros que va a tener el tanque en el próximo minuto (o llamada recursiva)
    (define minutoSiguiente (+ minutosTranscurridos 1))             ; Defino el minuto siguiente para pasarlo como parámetro a la llamada recursiva
    (cond
